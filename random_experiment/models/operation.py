...
OPS = {
    'conv_1x1': lambda  C_in, C_out, stride, affine: ReLUConvBN(C_in, C_out, 1, stride, 0, affine=affine),
    ...
    'avg_pool_3x3': lambda C_in, C_out, stride, affine: nn.AvgPool2d(3, stride=stride, padding=1,
                                                                     count_include_pad=False) if C_in == C_out else nn.Sequential(
        nn.AvgPool2d(3, stride=stride, padding=1, count_include_pad=False),
        nn.Conv2d(C_in, C_out, 1, stride=1, padding=0, bias=False),
        nn.BatchNorm2d(C_out, eps=1e-3, affine=affine)
    ),
    'max_pool_3x3': lambda C_in, C_out, stride, affine: nn.MaxPool2d(3, stride=stride,
                                                                     padding=1) if C_in == C_out else nn.Sequential(
...
class FactorizedReduce(nn.Module):
    def __init__(self, C_in, C_out, affine=True):
        ...
        #     nn.Conv2d(C_in, C_out // 2, 1, stride=1, padding=0, bias=False)
        # )
